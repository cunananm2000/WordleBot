...
def genStrategyTree(candidates, validGuesses, valuation, depth=0, searchDepth=0):
    ...
    _, guess = getBestGuess(
        candidates, validGuesses, valuation, depth, searchDepth=searchDepth
    )
    ...
    for response, split in getSplits(guess, candidates, useWords=True).items():
        ...
        res["splits"][response] = genStrategyTree(
            split, validGuesses, valuation, depth=depth + 1, searchDepth=searchDepth
        )
...
def writeStrategyTree(v, common, searchDepth=0):
    if common:
        tree = genStrategyTree(
            commonWords, guesses + answers, v, searchDepth=searchDepth
        )
    else:
        tree = genStrategyTree(
            guesses + answers, guesses + answers, v, searchDepth=searchDepth
        )

    with open(
        f"{'commonTrees' if common else 'trees'}/{v.__name__}{searchDepth + 1}.json",
        "w",
    ) as f:
...
if __name__ == "__main__":
    writeStrategyTree(v=information, common=True, searchDepth=1)
